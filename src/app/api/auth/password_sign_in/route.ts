...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!responseData.credentials?.access_token) {
      console.error('No access token in response:', responseData);
      return NextResponse.json(
        { error: 'Invalid response from Pi server' },
        { status: 500 }
      );
    }
    
    // Set cookies with account information and bearer token
    ...
    (await cookieStore).set('pi_account', JSON.stringify(accountData), {
      httpOnly: true,
