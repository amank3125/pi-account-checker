...
export default function AccountPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!account) {
          setErrors({ pi: 'Account not found', user: 'Account not found', kyc: 'Account not found' });
          return;
        }
        ...
        // Fetch KYC data
        try {
          const kycResponse = await fetch("https://socialchain.app/api/kyc/pi_kyc_status", {
            method: 'GET',
            headers: headers,
            redirect: 'follow' as RequestRedirect
          });
  ...
  const retryPiData = async (account: unknown) => {
    try {
      const accessToken = (account as { credentials?: { access_token: string }, access_token?: string }).credentials?.access_token || (account as { access_token?: string }).access_token;
      const headers = new Headers();
      headers.append("Authorization", `Bearer ${accessToken}`);
      headers.append("Content-Type", "application/json");
  ...
  const ErrorWithRetry = ({ message, onRetry }: { message: string; onRetry: () => void }) => (
        ...
        <div className="p-8">
          <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
            <div className="bg-blue-600 p-4">
              <h1 className="text-2xl font-bold text-white text-center">Account Details</h1>
            </div>
            
            <div className="p-6 space-y-8">
              {/* Pi Mining Details */}
              <div className="space-y-4">
                <h2 className="text-xl font-semibold text-gray-800 border-b pb-2">Mining Details</h2>
                {errors.pi ? (
                  <ErrorWithRetry 
                    message={errors.pi} 
                    onRetry={async () => {
              ...
              {/* KYC Details */}
              <div className="space-y-4">
                <h2 className="text-xl font-semibold text-gray-800 border-b pb-2">KYC Status</h2>
                {errors.kyc ? (
                  <ErrorWithRetry 
                    message={errors.kyc} 
                    onRetry={async () => {
