...
export default function Home() {
  ...
  // Check if a single phone number has a Pi account
  const checkPhoneNumber = async (number: string): Promise<CheckResult> => {
    try {
      ...
      // Some responses have data.continue_in_webview_ui.path = '/signin/password'
  ...
  const handleBulkCheck = async () => {
    ...
    for (let i = 0; i < numbers.length; i++) {
      ...
      results.push(result);
      setProgress(i + 1);
      setBulkResults([...results]); // Spread to trigger state update

      // Short delay to help avoid rate-limiting
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen">
      ...
      <div className="md:hidden bg-blue-600 text-white p-4 flex justify-between items-center">
        <h1 className="text-xl font-bold">Pi Account Checker</h1>
                    ...
                    >
                      <h3
                        className={`text-lg font-semibold ${
                          isAccountFound ? 'text-green-800' : 'text-red-800'
                        }`}
                      >
                      ...
                      <p
                        className={`mt-1 ${
                          isAccountFound ? 'text-green-700' : 'text-red-700'
                        }`}
                      >
                        {isAccountFound
                    ...
                    <div className="mt-4">
                      <div className="w-full bg-gray-200 rounded-full h-2.5">
                        <div
                          ...
                          style={{
                            width: `${(progress / totalChecks) * 100}%`,
                          }}
                        />
                      </div>
