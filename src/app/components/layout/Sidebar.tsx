...
import { IconSearch, IconUsers, IconChevronDown, IconChevronRight, IconChevronLeft, IconArrowRight } from '@tabler/icons-react';
    ...
    <>
      ...
      <div
        className={`fixed top-0 left-0 z-50 h-screen bg-gray-800 text-white transform
                    ...
                    ${isExpanded ? 'w-56 sm:w-60 md:w-64 min-w-[14rem]' : 'md:w-20'}`}
      >
        ...
        <div className="text-xl font-bold mb-6 text-center pt-6 px-2">
          {isExpanded ? (
            <span className="text-ellipsis overflow-hidden whitespace-nowrap">Pi Account Checker</span>
          ) : (
            <span className="hidden md:inline-block text-center w-full">Pi</span>
          )}
        </div>
        ...
        <nav className="space-y-2 px-2 overflow-y-auto h-[calc(100vh-180px)] scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent hover:scrollbar-thumb-gray-500 scrollbar-thumb-rounded-full">
          ...
          >
            ...
            {isExpanded && <span className="text-sm md:text-base">Checker</span>}
          </Link>
          <div>
            <div className="flex items-center">
              ...
              >
                <IconUsers className={`w-5 h-5 flex-shrink-0 ${isExpanded ? 'mr-2' : ''}`} />
                {isExpanded && <span className="text-sm md:text-base truncate">Manage Accounts
